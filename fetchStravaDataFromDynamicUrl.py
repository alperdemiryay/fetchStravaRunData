...
def fetch_strava_data():
    ...
    if fetchTheLatestActivity:
        log("Step 1: Fetching latest activity from training_activities API...")
        try:
            activities_url = "https://www.strava.com/athlete/training_activities?keywords=&sport_type=&tags=&commute=&private_activities=&trainer=&gear=&search_session_id=ed3e3324-58d6-4ebf-8209-b3397bbbcbdc&new_activity_only=false"
            resp = session.get(activities_url, timeout=10, verify=False)
            ...
            elif isinstance(activities_data, dict) and 'models' in activities_data and len(activities_data['models']) > 0:
                latest_activity = activities_data['models'][0]
            else:
                log("❌ No activity found in the response.")
                return

            latest_id = str(latest_activity['id'])
            if 'start_time' in latest_activity:
                # "2026-04-14T12:12:13+0000" -> "2026-04-14"
                activityDate = latest_activity['start_time'].split('T')[0]
            else:
                activityDate = latest_activity.get('start_date', activityDate)
                
            log(f"✅ Fetched Latest Activity: ID={latest_id}, Date={activityDate}")
            
        except Exception as e:
            log(f"Failed to fetch activities list: {e}")
            return
    else:
